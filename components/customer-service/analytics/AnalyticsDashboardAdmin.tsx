...
import { DatePicker, Select, Card, Space, message, Spin } from 'antd';
...
import type { AnalyticsResponse, AnalyticsParams } from './types';
          ...
          <div className="analytics-stats">
            <div className="stat-group">
              <div className="stat-item">
                ...
                <span className={`stat-value ${analyticsData.weeklyGrowth >= 0 ? 'trend-up' : 'trend-down'}`}>
                  {analyticsData.weeklyGrowth >= 0 ? '+' : ''}{analyticsData.weeklyGrowth}%
                </span>
              </div>
              <div className="stat-item">
                <span className="stat-label">月同比</span>
                <span className={`stat-value ${analyticsData.monthlyGrowth >= 0 ? 'trend-up' : 'trend-down'}`}>
          ...
          <div className="analytics-charts">
            <Card className="chart-card">
              ...
              <ResponsiveContainer width="100%" height={300}>
                <LineChart
                  data={[
                    { name: language === 'zh-CN' ? '周一' : 'Mon', conversations: analyticsData.conversations * 0.8, satisfaction: analyticsData.satisfaction * 0.9 },
                    { name: language === 'zh-CN' ? '周二' : 'Tue', conversations: analyticsData.conversations * 0.9, satisfaction: analyticsData.satisfaction * 0.95 },
                    { name: language === 'zh-CN' ? '周三' : 'Wed', conversations: analyticsData.conversations, satisfaction: analyticsData.satisfaction },
                    { name: language === 'zh-CN' ? '周四' : 'Thu', conversations: analyticsData.conversations * 1.1, satisfaction: analyticsData.satisfaction * 1.02 },
                    { name: language === 'zh-CN' ? '周五' : 'Fri', conversations: analyticsData.conversations * 1.2, satisfaction: analyticsData.satisfaction * 1.05 },
                    { name: language === 'zh-CN' ? '周六' : 'Sat', conversations: analyticsData.conversations * 0.7, satisfaction: analyticsData.satisfaction * 0.98 },
                    { name: language === 'zh-CN' ? '周日' : 'Sun', conversations: analyticsData.conversations * 0.6, satisfaction: analyticsData.satisfaction * 0.97 }
                  ]}
                >
